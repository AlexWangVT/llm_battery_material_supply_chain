...
serp_api_key = config["api"]["Serp_api"]
...
def search_load_data(online_data_sources, local_pdf_database):
    with open(online_data_sources, "r") as file:
        local_web_sources = [line.strip() for line in file.readlines() if line.strip()]

    pdf_sources = glob.glob(os.path.join(local_pdf_database, "*.pdf"))

    if source_filter == None:
        lines = local_web_sources + pdf_sources    
    else:
...
def main():
    ...
    mode = st.selectbox("Select Mode", ["💬 Q&A Mode (default)", "💼 Data Processing Mode", "🛠️ Model Finetuning"])
